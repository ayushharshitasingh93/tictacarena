...
router.get('/stats', async (req, res) => {
  try {
    ...
    const { count: activeToday } = await supabase.from('profiles').select('*', { count: 'exact', head: true }).eq('online', true);
    const today = new Date(); today.setHours(0,0,0,0);
    const { count: matchesToday } = await supabase.from('matches').select('*', { count: 'exact', head: true }).gte('created_at', today.toISOString());
    const { count: newUsers } = await supabase.from('profiles').select('*', { count: 'exact', head: true }).gte('created_at', today.toISOString());

    // Extended Feature Stats
    const { count: totalFriendships } = await supabase.from('friends').select('*', { count: 'exact', head: true });
    const { count: totalMessages } = await supabase.from('messages').select('*', { count: 'exact', head: true });
    const { count: totalAchievementsUnlocked } = await supabase.from('user_achievements').select('*', { count: 'exact', head: true }).eq('unlocked', true);
    const { count: totalMailsSent } = await supabase.from('mails').select('*', { count: 'exact', head: true });
    const { count: totalShopPurchases } = await supabase.from('user_items').select('*', { count: 'exact', head: true });
    ...
    res.json({
      ...
      totalMailsSent: totalMailsSent || 0,
...
router.get('/stats/extended', async (req, res) => {
  try {
    ...
    // 1. Fetch RAW data (minimize queries but get enough for aggregations)
    // In a real prod environment with millions of rows, we'd use SQL RPCs or views. 
    // Here we'll do lightweight JS aggregation on recent/relevant data.
    const { data: profiles } = await supabase.from('profiles').select('id, created_at, last_seen, coins, xp, score, wins, losses, rank, online, username');
    const { data: matches } = await supabase.from('matches').select('id, created_at, mode').gte('created_at', thirtyDaysAgo.toISOString());
    const { data: shopItems } = await supabase.from('shop_items').select('id, type, price');
    const { data: purchases } = await supabase.from('user_items').select('id, purchased_at, item_id').gte('purchased_at', thirtyDaysAgo.toISOString());
    ...
    // --- Core existing counts ---
    const totalUsers = profs.length;
    const totalMatchesAll = mat.length; // From last 30d for speed, could be a distinct count query
    const onlineNow = profs.filter(p => p.online).length;

    // --- Matches & Users Per Day (last 7 days) ---
    const matchesPerDay = [];
    const usersPerDay = [];
    for (let i = 6; i >= 0; i--) {
    ...
    // ============================================
    // NEW 21 CHART AGGREGATIONS
    // ============================================

    // 1: Revenue (Line) - simulate coins spent in shop last 7 days as "revenue"
    const revenueLabels = [];
    const revenueData = [];
    for (let i = 6; i >= 0; i--) {
      const d = new Date(); d.setDate(d.getDate() - i); d.setHours(0,0,0,0);
    ...
    // 2: Region (Bar) - Simulated based on timestamps (just for visual representation without IP tracking)
    ...
    // 3: Device Types (Doughnut) - Simulated
    ...
    // 6: Match Durations - Simulated distribution
    ...
    // 8: Retention (Day 1, 3, 7) - Simulated dropoff
    ...
    // 9: Churn Rates - Simulated over last 4 weeks
    const chart9 = { labels: ['W1', 'W2', 'W3', 'W4'], data: [4.5, 4.2, 5.1, 3.8] };

    // 10: ARPU - Revenue / Users
    const arpu = totalUsers > 0 ? (purch.length * 100) / totalUsers : 0; // rough simulation
    const chart10 = { labels: ['Current ARPU'], data: [arpu] };

    // 11: Premium Conversion (no is_premium column, use level > 10 as proxy)
    ...
    // 12: Support Tickets - Simulated
    const chart12 = { labels: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri'], data: [12, 8, 15, 10, 5] };

    // 13: Peak Concurrency - Simulated hourly distribution of active today
    ...
    // 21: Growth Proj (Linear extrapolation based on last 7 days)
...
router.get('/achievements', async (req, res) => {
  try {
    const { data } = await supabase.from('achievements_def').select('*').order('created_at');
    res.json(data);
  } catch (err) {
    res.status(500).json({ error: 'Server error' });
  }
...
router.post('/achievements', async (req, res) => {
  try {
    const { data, error } = await supabase.from('achievements_def').insert(req.body).select().single();
    if (error) return res.status(400).json({ error: error.message });
    res.json(data);
  } catch (err) {
    res.status(500).json({ error: 'Server error' });
...
router.put('/achievements/:id', async (req, res) => {
  try {
    const { data, error } = await supabase.from('achievements_def').update(req.body).eq('id', req.params.id).select().single();
    if (error) return res.status(400).json({ error: error.message });
    res.json(data);
  } catch (err) {
    res.status(500).json({ error: 'Server error' });
...
router.post('/rewards/send', async (req, res) => {
  try {
    ...
    if (send_to_all) {
      const { data: users } = await supabase.from('profiles').select('id');
      for (const u of (users || [])) { await rewardUser(u.id); }
      res.json({ success: true, rewarded: (users || []).length });
    } else {
      if (!user_id) return res.status(400).json({ error: 'user_id required' });
...
router.get('/activity-log', async (req, res) => {
  try {
    // Recent signups
    const { data: recentSignups } = await supabase.from('profiles')
      .select('id, username, created_at').order('created_at', { ascending: false }).limit(10);

    // Recent matches
    const { data: recentMatches } = await supabase.from('matches')
      .select('id, winner_id, player1_id, player2_id, created_at').order('created_at', { ascending: false }).limit(10);

    // Recent shop purchases
    const { data: recentPurchases } = await supabase.from('user_items')
      .select('id, user_id, item_id, purchased_at').order('purchased_at', { ascending: false }).limit(10);
